...
namespace XArmWrapper
{
  ...
  int __stdcall get_world_offset(fp32 pose[6]) {
      return arm->get_world_offset(pose);
  }

  int __stdcall get_tcp_offset(fp32 pose[6]) {
      return arm->get_tcp_offset(pose);
  }

  int __stdcall get_mode() {
      return arm->mode;
  }
}
