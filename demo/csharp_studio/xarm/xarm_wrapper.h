...
namespace XArmWrapper {
  ...
  extern "C" __declspec(dllexport) int __stdcall move_circle(fp32 pose1[6], fp32 pose2[6], fp32 percent, fp32 speed = 0, fp32 acc = 0, fp32 mvtime = 0, bool wait = false, fp32 timeout = NO_TIMEOUT, bool is_tool_coord = false, bool is_axis_angle = false);
  ...
  // O. PATROUIX Modification
  extern "C" __declspec(dllexport) int __stdcall get_world_offset(fp32 pose[6]);
  extern "C" __declspec(dllexport) int __stdcall get_tcp_offset(fp32 pose[6]);
  extern "C" __declspec(dllexport) int __stdcall get_mode();
}
