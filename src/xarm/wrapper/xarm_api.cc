...
int XArmAPI::get_world_offset(fp32 pose[6]) {
    if (!is_connected()) return API_CODE::NOT_CONNECTED;
    int ret = 1;
    for (int i = 0; i < 6; i++) {
        if (!default_is_radian && i > 2) {
            pose[i] = to_degree(this->world_offset[i]);
        }
        pose[i] = this->world_offset[i];
    }
    return ret;
}

int XArmAPI::get_tcp_offset(fp32 pose[6]) {
    if (!is_connected()) return API_CODE::NOT_CONNECTED;
    int ret = 1;
    for (int i = 0; i < 6; i++) {
        if (!default_is_radian && i > 2) {
            pose[i] = to_degree(this->tcp_offset[i]);
        }
        pose[i] = this->tcp_offset[i];
    }
    return ret;
}
